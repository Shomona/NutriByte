...
public class NutriProfiler {
	...
	static void createNutriProfile(Person person) {
		...
		recommendedNutrientsList.add(new RecommendedNutrient(ENERGY_NUTRIENT_CODE, energy));
		...
		for(int index=0 ; index<nutri.length; index++)
		{
			for( NutriEnum nutriEnum : NutriEnum.values()) {
				...
				if(nutriEnum.getNutriIndex() == index) {
					recommendedNutrientsList.add(new RecommendedNutrient(nutriEnum.getNutrientCode(), nutri[index]));
				}
			}
		}
	}
