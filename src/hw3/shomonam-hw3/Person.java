...
public abstract class Person {
	...
	public void populateDietNutrientMap() {
		...
		for(Product current : this.dietProductsList) {
			ObservableMap<String, Product.ProductNutrient> nutrientMap = current.getProductNutrients();
			float dietServingSize = current.getServingSize();
			
			//float productServingSize = NutriByte.model.productsMap.get(current.getNdbNumber()).getServingSize();
			
			float factor = dietServingSize / 100;
			
			for (String id : nutrientMap.keySet()) {

				Product.ProductNutrient currentNutrient = nutrientMap.get(id);
				if(this.dietNutrientsMap.containsKey(id)) {

					RecommendedNutrient dietMapNutrient = this.dietNutrientsMap.get(id);
					dietMapNutrient.setNutrientQuantity(dietMapNutrient.getNutrientQuantity() + (currentNutrient.getNutrientQuantity() * factor));
					this.dietNutrientsMap.put(id, dietMapNutrient);
				}
				else {
					this.dietNutrientsMap.put(id, new RecommendedNutrient(currentNutrient.getNutrientCode(), currentNutrient.getNutrientQuantity() * factor));
				}
