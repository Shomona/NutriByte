...
public class NutriProfiler {
	...
	static void createNutriProfile(Person person) {
		...
		person.recommendedNutrientsList.add(new RecommendedNutrient(ENERGY_NUTRIENT_CODE, energy));

		float[] nutri = person.calculateNutriRequirement();

		for(int index=0 ; index<nutri.length; index++)
		{
			for( NutriEnum nutriEnum : NutriEnum.values()) {

				if(nutriEnum.getNutriIndex() == index) {
					person.recommendedNutrientsList.add(new RecommendedNutrient(nutriEnum.getNutrientCode(), nutri[index]));
				}
			}
		}
	}
